...
import { getUsersListL } from "../../api";
import "./Messenger.css";
import { IoSend, IoClose, IoArrowBack, IoEllipsisVertical, IoTrash } from "react-icons/io5";
...
export default function Messenger({ isOpen, onClose }) {
  const { conversations, messages, currentConversation, setCurrentConversation, sendMessage, fetchMessages, fetchConversations, deleteConversation } = useMessaging();
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (isOpen) {
      fetchConversations();
      // Load all users
      getUsersListL().then(res => {
        if (res?.data) {
          // Filter out current user and map to standard format
          const currentUserId = user?.id || user?._id;
          const users = res.data
            .filter(u => (u._id || u.id) !== currentUserId)
            .map(u => ({
              ...u,
              id: u._id || u.id // Ensure id is always available
            }));
          setAllUsers(users);
        }
      }).catch(err => console.error("Failed to load users:", err));
    }
  }, [isOpen, fetchConversations, user]);
  ...
  const handleSendMessage = async () => {
    ...
    const currentUserId = user?.id || user?._id;
    const recipientId = currentConversation.participants.find((p) => p !== currentUserId);
    ...
    <div className="messenger-overlay" onClick={onClose}>
      <div className="messenger-container" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="messenger-body">
              ...
              filteredConversations.map((conv) => (
                ...
                >
                  ...
                  <div className="conv-info" >
                    <h4>Conversationss {conv.id.slice(0, 8)}</h4>
          ...
          {/* Messages View */}
          <div className={`messages-view ${currentConversation ? 'active' : ''}`}>
            {currentConversation ? (
              <>
                <div className="messages-header">
                  ...
                  <h3>Conversation {currentConversation.id.slice(0, 8)}</h3>
                </div>
